...
public class Feature_jan {
  static Map<String, Map<Integer, List<Integer>>> Qterm_Doc_Positions =
      new HashMap<>();
  static Map<Pair<String, String>, Integer> CoccuranceMap = new HashMap<>();

  public static Map<String, Double> feature1(IndexReader index,
      Map<String, Double> ql_map,
      List<SearchResult> ser) throws IOException {
    // System.out.println(ql_map.size());
    double denom = 0;
    // String pathStopwords = "C:\\Users\\jannu
    // bhai\\IdeaProjects\\FinalProject\\stopwords_inquery";
    // LuceneQLSearcher searcher=new LuceneQLSearcher(pathStopwords);
    // searcher.setStopwords(pathStopwords);

    /*
     * for(String expanded:ql_map.keySet()) { System.out.println(expanded); }
     */
    double tot_freq = 0;
    ...
    for (String expand_terms : ql_map.keySet()) {
      ...
      for (int i = 0; i < ser.size(); i++) {
        ...
        for (Map.Entry<String, Double> word_freq : Docwise_wordfreq_map
            .get(docid).entrySet()) {
  ...
  public static Map<String, Double> feature3(IndexReader index,
      Map<String, Double> ql_map, List<SearchResult> ser, List<String> qterms)
      throws IOException {
    ...
    for (String term : ql_map.keySet()) {
      ...
      PostingsEnum posting = MultiFields.getTermDocsEnum(index, "content",
          new BytesRef(term), PostingsEnum.POSITIONS);
      double score_of_n_qterms = 0;
      for (String queryTerms : Qterm_Doc_Positions.keySet()) {
        int Coccurance_tot = 0;
        Map<Integer, List<Integer>> MapList_of_Doc_position =
            Qterm_Doc_Positions.get(queryTerms);
        int Coccurance_count = 0;
        if (posting != null) { // if the term does not appear in any document,
                               // the posting object may be null
          int docid;
        ...
        CoccuranceMap.put(new Pair<String, String>(queryTerms, term),
            Coccurance_count);
      ...
      double score = Math.log(
          (double) score_of_n_qterms / Qterm_Doc_Positions.keySet().size());
  ...
  public static void QueryDocumentPosition(List<String> qterms,
      IndexReader index,
      List<Integer> FBDocList) throws IOException {
    for (String Qterm : qterms) {
      PostingsEnum post_qterm = MultiFields.getTermDocsEnum(index, "content",
          new BytesRef(Qterm), PostingsEnum.POSITIONS);
  ...
  public static Map<String, Double> feature5(IndexReader index,
      Map<String, Double> ql_map,
      List<SearchResult> ser, List<String> qterms) throws IOException {
    List<Integer> FeedbackDocList = new ArrayList<>();
    int numfb = 20;

    Map<String, Double> feat5 = new HashMap<String, Double>();
    for (int i = 0; i < Math.min(numfb, ser.size()); i++) {
      FeedbackDocList.add(ser.get(i).getDocid());
    }
    double tot_freq = 0;
    ...
    for (int i = 0; i < qterms.size(); i++) {
      Map<Integer, List<Integer>> Term1_Doc_Pos =
          Qterm_Doc_Positions.get(qterms.get(i));
      Set<Integer> Term1_DocID_Set = new HashSet<>(Term1_Doc_Pos.keySet());
      for (int j = i + 1; j < qterms.size(); j++) {
        Map<Integer, List<Integer>> Term2_Doc_Pos =
            Qterm_Doc_Positions.get(qterms.get(j));
    ...
    for (String ExpTerms : ql_map.keySet()) {
      PostingsEnum postingExpTerm = MultiFields.getTermDocsEnum(index,
          "content", new BytesRef(ExpTerms), PostingsEnum.POSITIONS);
      double score_of_n_qterms = 0;
      for (Pair p : pairSetMap.keySet()) {
        ...
        int Coccurance_count = 0;
        if (postingExpTerm != null) { // if the term does not appear in any
                                      // document, the posting object may be
                                      // null
          int docid;
          while ((docid =
              postingExpTerm.nextDoc()) != PostingsEnum.NO_MORE_DOCS) {
            if (FeedbackDocList.contains(docid)
                && pair_doc_list.contains(docid)) {
              List<Integer> posTerm1 =
                  Qterm_Doc_Positions.get(term1).get(docid);
              List<Integer> posTerm2 =
                  Qterm_Doc_Positions.get(term2).get(docid);
                ...
                for (int i = 0; i < freq; i++) {
                  ...
                  for (int k1 = 0; k1 < min; k1++) {
                    for (int k2 = 0; k2 < max; k2++) {
                      if (Math.abs(k1 - position_of_expansionTerm) <= 15
                          && Math.abs(k2 - position_of_expansionTerm) <= 15) {
                        Coccurance_count++;
                      }
                    }
                  }
      ...
      feat5.put(ExpTerms, score);
    }

    return feat5;
  }

  public static Map<String, Double> feature7(IndexReader index,
      Map<String, Double> ql_map,
      List<String> terms, List<SearchResult> ser) throws IOException {
    /*
     * for(String eterm:ql_map.keySet()) { System.out.println(eterm); }
     */

    Map<String, Double> feat7 = new HashMap<>();
    List<Integer> FeedbackDocList = new ArrayList<>();
    int numfb = 20;
    for (int i = 0; i < Math.min(numfb, ser.size()); i++) {
      FeedbackDocList.add(ser.get(i).getDocid());
    ...
    for (String qterm : terms) {
      ...
      for (String ExpTerm : ql_map.keySet()) {
        PostingsEnum posting = MultiFields.getTermDocsEnum(index, "content",
            new BytesRef(ExpTerm), PostingsEnum.POSITIONS);
        List<Integer> minDistList = new ArrayList<>();
        if (posting != null) {
          int docid;
          while ((docid = posting.nextDoc()) != PostingsEnum.NO_MORE_DOCS) {
            if (qtermPos.containsKey(docid)
                && FeedbackDocList.contains(docid)) {
              int freq = posting.freq();
              for (int i = 0; i < freq; i++) {
        ...
        if (minDistList.size() != 0)
          Qterm_MinDist_FD.put(new Pair<String, String>(qterm, ExpTerm),
              Collections.min(minDistList));
        // minDist=12;
      }
    }
  ...
  public static Map<String, Double> feature9(IndexReader index,
      List<SearchResult> ser,
      List<String> qterms, Map<String, Double> ql_map) throws IOException {
    List<Integer> FeedbackDocList = new ArrayList<>();
    Map<String, Double> feat9 = new HashMap<>();
    int numfb = 20;
    for (int i = 0; i < Math.min(numfb, ser.size()); i++) {
      FeedbackDocList.add(ser.get(i).getDocid());
    }
    ...
    for (String qterm : Qterm_Doc_Positions.keySet()) {
      if (i == 0) {
        ...
        commonDocId.retainAll(qtermDocid);// list of docids for all the qterms
                                          // together
      }
      i++;
    }
    for (String expTerm : ql_map.keySet()) {
      ...
      PostingsEnum posting =
          MultiFields.getTermDocsEnum(index, "content", new BytesRef(expTerm));
      if (posting != null) {
        int docid;
        while ((docid = posting.nextDoc()) != PostingsEnum.NO_MORE_DOCS) {
          if (FeedbackDocList.contains(docid)) {
      ...
      System.out.println("Score for " + expTerm + " : "
          + Math.log(expTermDocSet.size() + 0.5));
      feat9.put(expTerm, Math.log(expTermDocSet.size() + 0.5));
    }
  ...
  public static void main(String[] args) throws IOException {
    ...
    List<SearchResult> RM1res = searcher.search("content", terms, 1500.0, 20);
    Map<String, Double> scoresRM1 = searcher.estimateQueryModelRM1(field_search,
        terms, 1500.0, 0.0, 20, Integer.MAX_VALUE);

    Map<String, Double> feat1 = feature1(searcher.index, scoresRM1, RM1res);
    Map<String, Double> feat3 =
        feature3(searcher.index, scoresRM1, RM1res, terms);
    Map<String, Double> feat5 =
        feature5(searcher.index, scoresRM1, RM1res, terms);
    Map<String, Double> feat7 =
        feature7(searcher.index, scoresRM1, terms, RM1res);
    Map<String, Double> feat9 =
        feature9(searcher.index, RM1res, terms, scoresRM1);
