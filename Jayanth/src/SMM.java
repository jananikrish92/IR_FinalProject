...
/*
Simple mixture model using expectation maximization algorithm
 */
public class SMM {
  static Map<String, Integer> tfs = new HashMap<>();
  static int totalfbDocsLen = 0;
  public Map<String, Double> EM(Map<String, Integer> tfs, int totalfbDocsLen, Map<String, HashMap<Integer, Long>> wfreqs, IndexReader index, double lambda) throws IOException{
    ...
    do{
      ...
      for (String w : P.keySet()) {
        if (Math.abs(P.get(w) - prev.get(w)) < 0.001)
          count++;
        else
          prev.put(w, P.get(w));
      }
...
/*
 // for finding the query distribution of usefulness
  public static void  main(String args[])throws  IOException{
    ...
    double pwf=0,pwc=0;
    Map<String ,Double> mapLogpf = new TreeMap<>();
    Map<String ,Double> mapLogpwc = new TreeMap<>();
      System.out.println(totalfbDocsLen);
    for (String w : res.keySet()) {
      pwf = Math.log((double)tfs.get(w)/totalfbDocsLen);

      pwc = Math.log((double)searcher.index.totalTermFreq(new Term("content",w))/searcher.index.getSumTotalTermFreq("content"));
      mapLogpf.put(w,pwf);
      mapLogpwc.put(w,pwc);
    }
    mapLogpf = Features.scalingFeatures(mapLogpf);
    mapLogpwc = Features.scalingFeatures(mapLogpwc);
    for(String exp : res.keySet())
    {
     System.out.println(exp+" : "+"feedback log: "+mapLogpf.get(exp)+" collection log "+mapLogpwc.get(exp));
    }
  }
*/
