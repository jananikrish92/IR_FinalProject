...
public class SampleTest {
  public static void main(String[] args) throws IOException {
    ...
    String p = query_map.get("51");
    List<String> query = LuceneUtils.tokenize(p, ana);
    // we also print out external ID
    Set<String> fieldset = new HashSet<>();
    fieldset.add("docno");

    // The following loop iteratively print the lengths of the documents in the
    // index.
    ...
    for (int docid = 0; docid < ixreader.maxDoc(); docid++) {
      ...
      double corpusTF = ixreader
          .totalTermFreq(new Term("content", new BytesRef(query.get(1))));
      // double ctd = ter.totalTermFreq();
      for (int j = 0; j < query.size(); j++)
        System.out.println(docno + query.get(j) + " " + ixreader
            .docFreq(new Term("content", new BytesRef((query.get(j))))));
      // }
      ...
      // An acceptable but slow solution is that you calculate document length
      // by yourself based on
      // document vector. In case your dataset is static and relatively small
      // (such as about or less
      // than a few million documents), you can simply compute the document
      // lengths and store them in
      // an external file (it takes just a few MB). At running time, you can
      // load all the computed
      // document lengths to avoid loading doc vector and computing length.
      // TermsEnum termsEnum = ixreader.getTermVector( docid, field
      // ).iterator();
