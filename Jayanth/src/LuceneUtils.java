...
/**
 * Some Lucene utilities for CS646 students. Feel free to use them in your
 * homework.
 */
public class LuceneUtils {
    ...
    public static Analyzer getAnalyzer(final Stemming stemming) {
        return new Analyzer() {
            protected TokenStreamComponents createComponents(String fieldName) {
                TokenStreamComponents ts =
                        new TokenStreamComponents(new StandardTokenizer());
                ts = new TokenStreamComponents(ts.getTokenizer(),
                        new LowerCaseFilter(ts.getTokenStream()));
                if (stemming == Stemming.Krovetz) {
                    ts = new TokenStreamComponents(ts.getTokenizer(),
                            new KStemFilter(ts.getTokenStream()));
                } else if (stemming == Stemming.Porter2) {
                    ts = new TokenStreamComponents(ts.getTokenizer(),
                            new SnowballFilter(ts.getTokenStream(), "English"));
                }
                return ts;
            }
        };
    ...
    /**
     * Find a document in the index by its docno (external ID). Returns the
     * internal ID of the document; or -1 if not found.
     *
     * @param index An index reader.
     * @param fieldDocno The name of the field you used for storing docnos
     ...
     * @throws IOException
     */
    public static int findByDocno(IndexReader index, String fieldDocno,
                                  String docno) throws IOException {
        BytesRef term = new BytesRef(docno);
        PostingsEnum posting =
                MultiFields.getTermDocsEnum(index, fieldDocno, term, PostingsEnum.NONE);
        if (posting != null) {
     ...
     * @throws IOException
     */
    public static String getDocno(IndexReader index, String fieldDocno, int docid)
            throws IOException {
        // This implementation is just for you to quickly understand how this works.
        // You should consider reuse the fieldset if you need to read docnos for a
        // lot of documents.
     ...
     */
    public static List<String> tokenize(String text, Analyzer analyzer)
            throws IOException {
    ...
    private static Analyzer kstemmer = new Analyzer() {
        protected TokenStreamComponents createComponents(String fieldName) {
            TokenStreamComponents ts =
                    new TokenStreamComponents(new DoNothingTokenizer());
            ts = new TokenStreamComponents(ts.getTokenizer(),
                    new KStemFilter(ts.getTokenStream()));
            return ts;
        }
    ...
    /**
     * Well, this is kind of slow and probably not thread-safe, but Lucene's
     * Krovetz stemmer doesn't have a public constructor.
     *
     * @param word A word.
