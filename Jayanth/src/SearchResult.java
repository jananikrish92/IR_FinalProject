...
/**
 * SearchResult stores a search result, including its internal ID (docid),
 * external ID (docno), and its relevance score computed by the retrieval
 * system.
 */
public class SearchResult implements Comparable<SearchResult> {
     ...
     */
    public static void writeTRECFormat(PrintStream writer, String queryid,
                                       String runname, List<SearchResult> results, int n) {
        for (int ix = 0; ix < results.size() && ix < n; ix++) {
            int rank = ix + 1;
            writer.printf("%s 0 %s %d %.8f %s\n", queryid, results.get(ix).docno,
                    rank, results.get(ix).score, runname);
        }
    }
     ...
     * @throws IOException
     */
    public static Map<String, List<SearchResult>> readTRECFormat(File file)
            throws IOException {
    ...
    public static void dumpDocno(IndexReader index, String field_docno,
                                 Collection<SearchResult> results) throws IOException {
