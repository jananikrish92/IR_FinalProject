...
public class EvalUtils {
     ...
     */
    public static double precision(Collection<SearchResult> results,
                                   Set<String> relDocnos, int n) {
        double numrel = 0;
        int count = 0;
     ...
     */
    public static double recall(Collection<SearchResult> results,
                                Set<String> relDocnos, int n) {
        double numrel = 0;
        int count = 0;
        for (SearchResult result : results) {
     ...
     * @param relDocnos The set of judged relevant documents.
     * @param n The cutoff rank n (results at lower ranks will not be evaluated).
     * @return The average precision (AP) of the ranked list until some cutoff
     *         rank n.
     */
    public static double avgPrec(Collection<SearchResult> results,
                                 Set<String> relDocnos, int n) {
        double numrel = 0;
        double sumprec = 0;
        int count = 0;
    ...
    /**
     * Load a TREC-format relevance judgment (qrels) file (such as
     * "qrels_robust04" in HW2).
     *
     * @param f A qrels file.
     * @return A map storing the set of relevant documents for each qid.
     * @throws IOException
     */
    public static Map<String, Set<String>> loadQrels(String f)
            throws IOException {
