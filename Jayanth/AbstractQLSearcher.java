...
public abstract class AbstractQLSearcher {
  ...
  public abstract DocLengthReader getDocLengthReader(String field)
      throws IOException;
  ...
  public List<SearchResult> search(String field, List<String> terms, double mu,
  ...
  public List<SearchResult> search(String field,
      Map<String, Double> termWeights, double mu, int top) throws IOException {
    ...
    Collections.sort(postings,
        (p1, p2) -> postingWeights.get(p1).compareTo(postingWeights.get(p2)));
    weights.addAll(postings.stream().map(postingWeights::get)
        .collect(Collectors.toList()));
    return search(postings, weights, getDocLengthReader(field), mu, top);
  }

  private List<SearchResult> search(List<PostingList> postings,
      List<Double> weights, DocLengthReader doclen, double mu, int top)
      throws IOException {
    ...
    for (int ix = 0; ix < postings.size(); ix++) {
      ...
      if (!posting.end()) {
        ...
        length = merge(posting, weight, tf_smooth, smooth_unseen_sum,
            smooth_unseen, docs_old, scores_old, length_old, docs, scores);
  ...
  private int merge(PostingList posting, double pwq, double tf_smooth,
      double smooth_unseen_sum, double smooth_unseen, int[] docs_old,
      float[] scores_old, int length_old, int[] docs, float[] scores)
      throws IOException {
    int ix = 0;
    int ix_old = 0;
    while (!posting.end() && ix_old < length_old) {
      ...
      if (doc1 < doc2) {
        docs[ix] = doc1;
        scores[ix] +=
            smooth_unseen_sum + pwq * Math.log(posting.freq() + tf_smooth);
        ix++;
        posting.next();
      } else if (doc1 > doc2) {
        docs[ix] = doc2;
        ...
        docs[ix] = doc1;
        scores[ix] +=
            scores_old[ix_old] + pwq * Math.log(posting.freq() + tf_smooth);
        ix++;
        posting.next();
        ix_old++;
      }
